...
def polynomial_regression(x, y, degree, t):
    ...
    W = np.matmul(np.matmul(np.linalg.inv(
        np.matmul(x.T, x) + t * np.identity((x.shape[1]))), x.T), y)
...
def cross_validation(num_folds, x, y, degree, ax, t):
    ...
    for train_index, test_index in kf.split(x):
        ...
        W = polynomial_regression(x_train, y_train, degree, t)
...
#  設定samples數量
samples = 15
#  根據線, 產生加了noise的樣本, x落在[0, 1]
x = np.linspace(0, 1, samples)
y = np.sin(2*np.pi*x) + np.random.normal(0, 0.04, samples)
...
# -------------------------------------------------------------------------------
# Polynomial regression 執行
# degree = 14
degree = 14
W_best_polynomial = polynomial_regression(x, y, degree, t)
...
cross_validation(5, x, y, degree, ax_1, t)
